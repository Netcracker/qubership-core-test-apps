...
scenario builds around itself (`ConsulAcl`, `ConsulClient`, `SigningKey`), and the dump printed when a scenario fails
(`StandDump`). A scenario is then only what differs: the login way, the ACL objects it needs, and what it asserts.
