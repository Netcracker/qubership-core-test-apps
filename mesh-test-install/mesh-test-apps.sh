...
install_helm_package() {
    ...
    if [[ "$MESH_TYPE" == "ISTIO" ]]; then
        echo "Run CR transformer to prepare $service_name CRs for ISTIO mesh"
        transform_crs "$chart_path"
    else
        echo "Using CORE mesh type for $service_name installation"
    fi 
...
transform_routing_to_istio() {
    echo "Transforming routing to ISTIO for all services..."
...
transform_crs() {
    local chart_path=$1
    java -jar target/remesh-1.0.0.jar -d $chart_path/templates/
    find "$chart_path/templates" -type f -name '*-autogenerated.yaml' -print0 |
    while IFS= read -r -d '' file; do
        dir="$(dirname "$file")"
        base="$(basename "$file")"

        original_name="${base%-autogenerated.yaml}.yaml"
        original="$dir/$original_name"
        ...
        if [ -f "$original" ]; then
            echo "Removing original file: $original"
            rm -f "$original"
        fi
    done
}
...
clone_or_update_repo() {
	if [ -d "${PROJECT_ROOT}/$1" ]; then
		echo "Updating $1..."
		cd $PROJECT_ROOT/$1 && git pull
	else
		echo "Cloning $1..."
		if [ -n "$3" ]; then
			git clone --depth 1 -b $3 $2 ${PROJECT_ROOT}/$1
		else
			git clone --depth 1 $2 ${PROJECT_ROOT}/$1
		fi
	fi
}

install_mesh_cr_transformer() {
    echo "Installing mesh CR transformer..."
