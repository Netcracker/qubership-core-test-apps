...
install_services() {
    ...
    # Prepare CRs for ISTIO mesh installation if needed
    if [[ "$MESH_TYPE" == "ISTIO" ]]; then
        ...
        for chart in "$SPRING_CHART" "$QUARKUS_CHART" "$GO_CHART"; do
            java -jar target/remesh-1.0.0.jar -d $chart/templates/
            find "$chart/templates" -type f -name '*-autogenerated.yaml' -print0 |
