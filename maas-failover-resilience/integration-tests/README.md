# MaaS failover resilience tests
...
```
 test (runner) ──drives──► maas-failover-test-service-* ──uses──► client library ──► storage
       │                            │
...
(`SpringFailoverITBase`, `GoFailoverITBase`) and decides which application is
port-forwarded; the storage comes from `FailoverProfile` and carries the probe name, the thresholds
...
|---|---|---|
| `MaasFailoverIT` | spring | Java MaaS client |
| `MaasGoFailoverIT` | go | Go MaaS client and maas-core |
...
Found by `MaasRabbitGoFailoverIT`, fixed in
[maas-client#89](https://github.com/Netcracker/qubership-core-lib-go-maas-client/pull/89). The Go
client posted the bare classifier to `/api/v1/rabbit/vhost`:
...
so every call was rejected with 400 and `Field validation for 'Name' failed on the 'required' tag`.
The Java client wraps it — `post(new VHostRequest(classifier))` — and works. Only get-or-create was
...
Fixed in [maas-service#302](https://github.com/Netcracker/qubership-maas/pull/302).
...
`FailoverAssertions.assertContract` checks all four at once:
...
mvn -f $suite/test-apps-java/pom.xml package -DskipTests
docker build -t maas-failover-test-service-spring:it -f $suite/test-service-spring/Dockerfile $suite/test-service-spring
docker build -t maas-failover-test-service-go:it -f $suite/test-service-go/Dockerfile $suite/test-service-go
kind load docker-image maas-failover-test-service-spring:it maas-failover-test-service-go:it --name kind
...
`maas-failover-resilience/install/manifests/test-app.yaml`, rendered per platform with `envsubst`. They
