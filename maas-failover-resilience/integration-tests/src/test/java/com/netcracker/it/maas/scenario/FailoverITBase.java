...
import com.netcracker.it.maas.app.MaasTestApp;
import com.netcracker.it.maas.app.WorkloadStats;
import com.netcracker.it.maas.controller.FaultController;
...
import static com.netcracker.it.maas.scenario.FailoverAssertions.assertContract;
import static com.netcracker.it.maas.scenario.FailoverAssertions.assertNoLeak;
import static com.netcracker.it.maas.scenario.FailoverAssertions.assertNothingHung;
...
public abstract class FailoverITBase {

    private static final Logger log = LoggerFactory.getLogger(FailoverITBase.class);
    ...
    protected abstract FailoverProfile profile();

    @BeforeEach
    void setUpFixture() {
        requireServices();
        app = new MaasTestApp(appUrl());
    ...
    protected WorkloadStats runWorkloadThrough(Fault fault, String handleMode) {
        ...
        log.info("{} / {}: {}", profile().probe(), fault, FailoverAssertions.summarise(stats, faultClearedAt));
        return stats;
    }
    ...
    private void awaitRecovery() {
        try {
            await("the client answers again")
                    ...
                    .until(() -> FailoverAssertions.firstSuccessAfter(app.stats(), faultClearedAt).isPresent());
