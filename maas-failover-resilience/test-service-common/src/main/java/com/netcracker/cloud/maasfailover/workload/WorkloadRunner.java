package com.netcracker.cloud.maasfailover.workload;
...
public class WorkloadRunner {
    ...
    private final Map<String, MaasProbe> probes;
    ...
    public WorkloadRunner(List<MaasProbe> probes) {
        this.probes = probes.stream().collect(Collectors.toMap(MaasProbe::type, probe -> probe));
    }

    /** Every probe this runner knows, for the diagnostics endpoint. */
    public List<MaasProbe> probes() {
        return List.copyOf(probes.values());
    }

    public MaasProbe probe(String storageType) {
        MaasProbe probe = probes.get(storageType);
        if (probe == null) {
    ...
    public synchronized void start(String storageType, HandleMode mode, int operationsPerSecond) {
        stop();
        MaasProbe probe = probe(storageType);
        probe.init();
