...
func NewRunner(probes ...probe.Probe) *Runner {
	byType := make(map[string]probe.Probe, len(probes))
...
func (r *Runner) Probe(storageType string) (probe.Probe, error) {
...
func (r *Runner) Probes() []probe.Probe {
	probes := make([]probe.Probe, 0, len(r.probes))
...
func (r *Runner) Start(ctx context.Context, storageType string, mode probe.HandleMode, operationsPerSecond int) error {
...
func (r *Runner) loop(ctx context.Context, done chan struct{}, probe probe.Probe,
	mode probe.HandleMode, period time.Duration) {
	defer close(done)
	for {
...
func (r *Runner) runOnce(ctx context.Context, probe probe.Probe, mode probe.HandleMode) {
	r.outcomeMu.Lock()
	r.sequence++
	sequence := r.sequence
	r.outcomeMu.Unlock()
