...
# Installs the failover test applications into a namespace.
#
...
#
# Usage: ./test-apps.sh [install|uninstall] [--namespace NS] [--apps LIST] [--tag TAG]
...
MANIFEST="$SCRIPT_DIR/manifests/test-app.yaml"
...
show_usage() {
    ...
    echo "  --spring-tag TAG     Image tag for maas-failover-test-service-spring, overrides --tag"
    echo "  --go-tag TAG         Image tag for maas-failover-test-service-go, overrides --tag"
...
service_name_of() {
    echo "maas-failover-test-service-$1"
}
...
case "$ACTION" in
    install)
        ...
        echo "  kubectl get pods -n $NAMESPACE -l app.kubernetes.io/part-of=maas-failover-test-app"
        ;;
    uninstall)
        check_tools
